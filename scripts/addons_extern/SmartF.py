...
class SmartFMenu(bpy.types.Panel):
    ...
    bpy.types.Scene.units_size = bpy.props.EnumProperty(items= (('0', 'm', 'meters'),    
                                                 ...
                                                 ('2', 'mm', 'millimeters')),                                                     
                                                 name = 'unit size')         
    bpy.types.Scene.switch_toF2 = bpy.props.BoolProperty(
      name="switch_toF2",
      description="Switch to F2 without SmatF functions",
      default = False )
    ...
    def draw(self, context):
      layout = self.layout
      scn = context.scene
      if not int(bpy.context.scene.switch_toF2): 
      ...
      layout.prop(scn,'switch_toF2',text='Switch to F2')          
...
class SmartF(bpy.types.Operator):    
    ...
    def invoke(self, context, event):
        ...
        if len(sel) > 2:
            ...
            if int(bpy.context.scene.switch_toF2): 
...
def register():    
    ...
    #F2 class    
    bpy.utils.register_class(F2AddonPreferences)
...
def unregister():    
    ...
    bpy.utils.unregister_class(F2AddonPreferences)
