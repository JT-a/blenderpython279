...
from toolplus_align.np_utils_geometry import *
from toolplus_align.np_utils_graphics import *
...
class NP020PDGetSelection(bpy.types.Operator):
    bl_idname = 'object.np_pd_get_selection_np'
    ...
    def execute(self, context):
        ...
        print ('NP020PD.acob =', NP020PD.acob)
...
class NP020PDReadMouseLoc(bpy.types.Operator):
    bl_idname = 'object.np_pd_read_mouse_loc_np'
...
class NP020PDAddPoints(bpy.types.Operator):
    bl_idname = 'object.np_pd_add_points_np'
...
def draw_callback_px_TRANS(self, context):
    ...
    if addon_prefs.nppd_col_num_shadow_DEF == False:
        ...
        col_num_shadow = (1.0, 1.0, 1.0, 0.65)
...
class NP020PDRunTranslate(bpy.types.Operator):
    bl_idname = 'object.np_pd_run_translate_np'
...
class NP020PDRunNavigate(bpy.types.Operator):
    bl_idname = "object.np_pd_run_navigate_np"
...
class NP020PDChangePhase(bpy.types.Operator):
    bl_idname = "object.np_pd_change_phase_np"
    ...
    def execute(self, context):
        ...
        NP020PD.phase = 1
        print('NP020PD.phase=', NP020PD.phase)
        start = NP020PD.start
        end = NP020PD.end
        startloc3d = start.location
        endloc3d = end.location
...
class NP020PDDeletePoints(bpy.types.Operator):
    bl_idname = "object.np_pd_delete_points_np"
    ...
    def execute(self, context):
        ...
        if step == 'simple':
            print('07_delete_points_END_cancelled')
            return {'CANCELLED'}
        if step == 'continuous':
            print('07_delete_points_END_FINISHED')
            return {'FINISHED'}
...
def register():
    ...
    for i in range(1, 15):
        NP020PointDistance.define('OBJECT_OT_np_pd_get_selection_np')
        NP020PointDistance.define('OBJECT_OT_np_pd_read_mouse_loc_np')
        NP020PointDistance.define('OBJECT_OT_np_pd_add_points_np')
        for i in range(1, 15):
            NP020PointDistance.define('OBJECT_OT_np_pd_run_translate_np')
            NP020PointDistance.define('OBJECT_OT_np_pd_run_navigate_np')
        NP020PointDistance.define('OBJECT_OT_np_pd_change_phase_np')
        for i in range(1, 15):
            NP020PointDistance.define('OBJECT_OT_np_pd_run_translate_np')
            NP020PointDistance.define('OBJECT_OT_np_pd_run_navigate_np')
        NP020PointDistance.define('OBJECT_OT_np_pd_delete_points_np')
