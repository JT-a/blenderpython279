...
from ui_tune_up.utils import dd, load_json, path
from ui_tune_up.keyboard import load_keyboard
dd(30*"-")


dirsep = os.path.sep
addons_folder = bpy.path.basename(path)
root = dirsep.join([path, "configs"])
#print("root", root)
...
def zipdir(basedir, archivename):
	...
	if not addons_folder:
		print("addon's folder not found")
		return
	
	#this is dangerous get the addons folder's name whatever that is
	ignored_folders = [addons_folder, "__pycache__"]
...
def setup_preferences():
	...
	return {"INFO"}, "Preferences loaded" + errors
...
class TUNEUP_OT_manage_configurations(bpy.types.Operator):
	...
	def execute(self, context):
		...
		if self.cmd == "LOAD_KEYBOARD":
			icon, msg = load_keyboard()
			self.report(icon, msg)
		
		elif self.cmd == "LOAD_PREFERENCES":
			icon, msg = setup_preferences()
			self.report(icon, msg)
