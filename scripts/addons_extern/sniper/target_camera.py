...
def getKeyframesFromAnimationDataEmpty(targets):
    ...
    if isValidKeyframeAmount(keyframes, targets):
        for i in range(len(targets)):
            ...
            if i > 0:
                positionBefore = max(keyframes[i * 2 - 1].co.x, 1)
            else:
                positionBefore = 0
            if i * 2 + 1 < len(keyframes):
                positionAfter = max(keyframes[i * 2 + 1].co.x, 1)
            else:
