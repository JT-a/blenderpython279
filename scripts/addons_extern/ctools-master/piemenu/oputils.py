...
def get_operator(string):
    ...
    if m:
        if 0:  # 例外を捉えられない場合がある
            ...
            name = m.group(2).upper() + "_" + m.group(3)
            try:
                _bpy.ops.get_rna(name)
            except KeyError:
                # KeyError: '_bpy.ops.get_rna("WM_OT_foo") not found
                pass
            else:
                op = getattr(getattr(bpy.ops, m.group(2)), m.group(3))
    return op
...
def get_operator_rna(string):
    m = re.match('(bpy\.ops\.)?(\w+)\.(\w+)(|\(.*\))$', string.rstrip())
    if m:
        try:
            return _bpy.ops.get_rna(m.group(2).upper() + "_OT_" + m.group(3))
        except KeyError:
            # KeyError: '_bpy.ops.get_rna("WM_OT_foo") not found
            return None
    return None
