...
def draw_scene_panel_layout(self, context, layout):
        ...
        if context.mode == 'OBJECT': 
            ...
            if scene.tp_panel_type == "layers":          
                ...
                row = box.row()
                col = row.column()
                col.template_list("layers_collection_UL", "", context.scene, "display_layers_collection", context.scene, "display_layers_collection_index", rows = 5)
                ...
                if tp_props.display_layer_id: 
                    sub.prop(tp_props, "display_layer_id", text="", icon="SCRIPTWIN")  
                else:                  
                    sub.prop(tp_props, "display_layer_id", text="", icon="SCRIPTWIN")          
                ...
                sub.prop(context.scene, "tp_modly_type", text="")
                sub1 = row.row(1)                 
                sub1.enabled = context.object.use_display_layer
                sub1.prop(context.scene, "tp_funcly_type", text="")
                ...
                row.operator("select_objects.btn", icon="RESTRICT_SELECT_OFF", text="")                                    
