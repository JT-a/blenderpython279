...
from toolplus_scene.layouts.normal_ui import draw_normal_layout
from toolplus_scene.layouts.smooth_ui import draw_smooth_layout
from toolplus_scene.layouts.screen_ui import draw_screen_layout
...
def draw_visual_layout(self, context, layout):
        ...
        if scene.tp_points == "tp_v1": 
            draw_screen_layout(self, context, layout) 
        ...
        if scene.tp_points == "tp_v2": 
            ...
            button_sphere_black = icons.get("icon_sphere_black")
            row.operator("tp_ops.toggle_silhouette", text="",icon_value=button_sphere_black.icon_id)      
        ...
        if scene.tp_points == "tp_v3": 
            ...
            if len(context.object.material_slots) > 0:               
                ...
                if context.object.active_material.use_object_color == True: 
                    ...
                    row.prop(context.object.active_material, "use_object_color","Mat-Color", icon="FILE_REFRESH")
                    sub = row.row(1)
                    sub.scale_x = 0.75 
                    sub.prop(context.object.active_material, "diffuse_color", text="")                
                
                row.operator("tp_ops.scene_to_all", text="", icon='PASTEDOWN').mode = "material, selected, append"                
                    ...
                    if ob: 
                        ...
                        if obj:               
                            if obj.show_bounds == True:
                                row.operator("tp_ops.showbound", text="Bounds on", icon='SNAP_PEEL_OBJECT')     
                            else:
                                row.operator("tp_ops.showbound", text="Bounds off", icon='SNAP_PEEL_OBJECT')        
                        else:
                            row.label("", icon="BLANK1")  
        ...
        if scene.tp_points == "tp_v5": 
            draw_smooth_layout(self, context, layout)
            

        if scene.tp_points == "tp_v6": 
            draw_normal_layout(self, context, layout)             
